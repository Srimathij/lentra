...
def extract_card_info():
    ...
    - Extract the Aadhaar Number, which is a 12-digit unique identity number issued by the Government of India.
    - The Aadhaar number may appear with or without spaces (e.g., "1234 5678 9012" or "123456789012").
    - The number may be split across multiple lines; in that case, reconstruct the digits before validation.
    - After removing spaces, the number must be exactly 12 digits long.
    - Ignore any numbers shorter or longer than 12 digits (e.g., 6-digit PIN codes, 16-digit VID numbers, or any other numeric identifiers).
    - Only digits are considered valid; ignore any alphabets or special characters mixed with the number.
    - If no valid Aadhaar number is found, return "Not Found".
